...
public class XTweenRotationTransform : XTweener 
{
	...
	{
		value = this.GetComponent<Transform>().eulerAngles;
	...
	{
        if (isLocal) {
			Vector3 tempMovement = transform.localEulerAngles;
			tempMovement = value;
            transform.localEulerAngles = tempMovement;
		} else {
